    ...
    try:
        ...
        if ind in trial.columns:
            shopee_rename_columns = {'Periode Data': 'DataPeriod',
                                     ...
                                     'Rata-rata durasi ditonton': 'AvgViewingDuration',
                                     'Penonton': 'Viewers',
...
def shopee_transform_action(shopee_path: str, result_path: str):
    ...
    ready_to_db_shopee_silver[ready_to_db_shopee_silver['live_start_date'] > datetime(2026, 8, 23).date()].to_csv(result_path+'ready_to_db_shopee_silver.csv', index=False)
    ...
    ready_to_db_shopee_silver[shopee_bronze_db][ready_to_db_shopee_silver['live_start_date'] > datetime(2026, 8, 23).date()].to_csv(result_path+'ready_to_db_shopee_bronze.csv', index=False)
...
def tiktok_transform_action(tiktok_path: str, result_path: str):
    ...
    ready_to_db_tiktok[ready_to_db_tiktok['live_start_date'] > datetime(2026, 8, 23).date()].to_csv(result_path+'ready_to_db_tiktok.csv', index=False)
